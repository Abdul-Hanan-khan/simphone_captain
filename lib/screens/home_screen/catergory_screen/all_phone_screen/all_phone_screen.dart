...
import 'package:sim_phone_captain/screens/home_screen/catergory_screen/all_phone_screen/phone_detail_screen/phone_detail_screen.dart';
...
class AllPhonesScreen extends StatefulWidget {
  const AllPhonesScreen(this.name, {Key? key}) : super(key: key);
  final String name;
...
class _AllPhonesScreenState extends State<AllPhonesScreen> {
  List<Mobiles> allDevices = [];
  final TextEditingController qty = TextEditingController();

  @override
  void initState() {
    context.read<AllPhonesCubit>().allPhones();
    super.initState();
  }

  // _showDialog(Mobiles data) {
  ...
  //                         } else {
  //                           context.read<AllPhonesCubit>().addDevice(id: data.id!, dType: data.deviceType!, qty: int.parse(qty.text));
  //                         }
  //                       },
  //                       child: Padding(
  ...
  Widget build(BuildContext context) {
    Size size = MediaQuery.of(context).size;
    return BlocConsumer<AllPhonesCubit, AllPhonesState>(
      listener: (context, state) {
        if (state is FailedToFetchedPhone) {
          showSnackBar(context, state.message);
        }
        if (state is FailedToOrderPhone) {
          showSnackBar(context, state.message);
        }
        if (state is AllPhonesFetchedSuccessfully) {
          allDevices.clear();
          allDevices = state.phones;
        }
                          ...
                          Row(
                            ...
                            children: [
                              ElevatedButton(
                                ...
                                onPressed: () {
                                  Nav.push(context, PhoneDetailScreen(allDevices[index]));
                                  // context.read<AllPhonesCubit>().orderMobile(productId: allDevices[index].id!, type: allDevices[index].deviceType!);
                                },
                                child: Padding(
                                  padding: EdgeInsets.all(size.height * 0.012),
                                  child: Text(
